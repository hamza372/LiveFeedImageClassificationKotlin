...
class MainActivity : AppCompatActivity(),ImageReader.OnImageAvailableListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            classifier = Classifier(this,Classifier.Device.CPU,1,"mobilenet_v1_1.0_224.tflite", "labels.txt")// Classifier(assets, "mobilenet_v1_1.0_224.tflite", "labels.txt", 224)
        } catch (e: IOException) {
            e.printStackTrace()
        }
    }
    ...
    private fun processImage() {
        ...
        val recognitionList =
            classifier!!.recognizeImage(rgbFrameBitmap!!,getScreenOrientation()) //classifier.recognizeImage(rgbFrameBitmap,getScreenOrientation());
        result = ""
        for (i in recognitionList.indices) {
            Log.d("tryLog", recognitionList[i].title!!)
